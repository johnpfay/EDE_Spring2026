...
#Import libraries
library(tidyverse);library(here);library(RColorBrewer)
library(sf)
library(leaflet)
...
ggplot(data=huc8_sf) + geom_sf(aes(fill=ACRES))
...
upper_neuse <- st_read(here('Data/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == 'Upper Neuse')

#Create a map
ggplot() + 
  geom_sf(data=huc8_sf, fill='gray') + 
  geom_sf(data=upper_neuse, color='blue',fill='yellow') 

mapview(huc8_sf,col.regions='gray') + 
  mapview(upper_neuse,col.regions='yellow',color='blue')
...
The data we'll add here is the CDC's Social Vulnerability Index data. Information on this dataset is available [here](https://www.atsdr.cdc.gov/place-health/php/svi/svi-data-documentation-download.html).
...
ggplot() + 
  ...
  geom_sf(data=epa_pm25_sites_sf,aes(size=maxPM)) +
  labs(fill='Socio\nEcononomic\nIndex')
...
wind_energy <- read.csv(
  file='https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  stringsAsFactors = TRUE,
  colClasses = c('HUC_12'='factor')
) %>% 
  mutate(HUC_8 = substr(HUC_12,0,8)) %>% 
  group_by(HUC_8) %>% 
...
#View the outputs
HUC_8_wind %>% ggplot + geom_sf(aes(fill=SumWindEnergy))
mapview(HUC_8_wind,zcol='SumWindEnergy')
...
mapview(huc8_sf,zcol='DWQ_Basin',legend=F)
...
mapview(triHUCs)+mapview(triCo,alpha.regions = 0,color='red')


#Intersect the HUC_8s
triHUCs_clipped <- huc8_sf_utm %>% st_intersection(triCo)
mapview(triHUCs_clipped)
...
#Map the results
mapView(counties_sf[resultMask,]) + mapview(theSite_sfc_transformed)
...
#Buffer the point 30km
theSite_sfc_transformed_buffered = st_buffer(theSite_sfc_transformed,30000)

#Create a boolean mask 
resultMask2 <- st_intersects(counties_sf_utm, 
                            theSite_sfc_transformed_buffered,
                            sparse = FALSE) 

#Filter the counties dataset using the boolean mask
selCounties2 <- counties_sf_utm[resultMask2,]

#Map the results
mapView(selCounties2) + mapview(theSite_sfc_transformed_buffered)
...
### 4.2.3 Leaflet - linked and sydata:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABIAAAASCAYAAABWzo5XAAAAbElEQVR4Xs2RQQrAMAgEfZgf7W9LAguybljJpR3wEse5JOL3ZObDb4x1loDhHbBOFU6i2Ddnw2KNiXcdAXygJlwE8OFVBHDgKrLgSInN4WMe9iXiqIVsTMjH7z/GhNTEibOxQswcYIWYOR/zAjBJfiXh3jZ6AAAAAElFTkSuQmCCnced plots
