...
#install.packages('arcgis') #<-- Does NOT work on containers
#library(arcgis)
...
https://catalog.data.gov/dataset/power-plants-81913
https://www.reddit.com/r/gis/comments/1mzu4np/300_hifld_datasets_archived/
...
When the data is served in GeoJSON format, we can ingest it directly in to R. Follow these steps: - Navigate to <https://gis-fws.opendata.arcgis.com/>
...
turtles <- st_read(
  paste0('https://services.arcgis.com/QVENGdaPbd4LUkLV/arcgis/rest/services/',
         'survey123_3d7e213f47a94f1b829aa7cf54eab095_results/FeatureServer/0/query?',
         'outFields=*&where=1%3D1&f=geojson'))

glimpse(turtles)

st_crs(powerplants)$epsg
```

#Now we can make a pretty map!
```{r cluster map}
#Create the map
cluster_map <- leaflet(turtles) %>% 
  addTiles() %>% 
  addMarkers(
...
data_source <- paste0(
  'https://maps.deq.nc.gov/arcgis/rest/services/',
  'DEMLR/NCGS_Landslide_Read_Vector/FeatureServer/0/',
  'query?outFields=*&where=1%3D1&f=geojson'
  )

data_source <- paste0(
  'https://webgis2.durhamnc.gov/server/rest/services/',
  'PublicServices/Community/MapServer/4/',
  'query?outFields=*&where=1%3D1&f=geojson')


sf_obj <- st_read(data_source) 

mapview(sf_obj,zcol='FACTYPE')
```
