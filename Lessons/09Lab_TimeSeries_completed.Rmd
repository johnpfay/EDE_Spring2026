...
BR_complete <- left_join(Months, BR_complete, by='Date')
...
BR_Temperature_ts = ts(BR_complete$Temperature_C,
                start=c(f_year,f_month),
                frequency=12)
...
BR_Temperature_Decomposed <- stl(BR_Temperature_ts, s.window='periodic')
plot(BR_Temperature_Decomposed)
...
BR_Rain_Components = data.frame(
  BR_Rain_Decomposed$time.series[,1:3]
)

BR_Temperature_Components = data.frame(
  BR_Temperature_Decomposed$time.series[,1:3]
)


BR_Rain_Components <- BR_Rain_Components %>% 
  mutate(Observed = BR_complete$Rainfall,
         Date = BR_complete$Date)

BR_Temperature_Components <- BR_Temperature_Components %>% 
...
BR_Rain_Components %>% ggplot() +
  geom_line(aes(y=Observed, x=Date), size=0.25) + 
  geom_line(aes(y=trend, x = Date), color='deeppink3') +
  #geom_line(aes(y=seasonal, x =Date), color='blue') + 
  #geom_hline(yintercept=0,lty=2) +
  ylab("Rainfall")


BR_Temperature_Components %>% ggplot() +
  geom_line(aes(y=Observed, x=Date), size=0.25) + 
  geom_line(aes(y=trend, x = Date), color='deeppink3') +
  #geom_line(aes(y=seasonal, x =Date), color='blue') + 
  #geom_hline(yintercept=0,lty=2) +
