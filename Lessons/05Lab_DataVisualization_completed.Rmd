...
* See <https://www.rdocumentation.org/packages/grDevices/versions/3.6.2/topics/plotmath>
...
#Plot how many Lake observations occur each month (bar plot)
...
PeterPaul.chem.nutrients %>% 
  mutate(fmonth = factor(month,levels=1:12,labels=month.abb)) %>% 
  ggplot(aes(x=fmonth)) + 
    geom_bar() + 
...
2. Plot the barplot with the reordered factor levels. Add this line of code to make the bars show up left to right: `scale_x_discrete(limits = function(x) rev(x))`
3. Adjust the x axis labels so they appear at rotated 90 degrees. 
